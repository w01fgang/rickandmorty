...
export const fetchEpisodes = async ({
  skip,
  size,
}: {| +skip: number, +size: number |}): Promise<Result<Error, Array<Episode>>> => {
  const range = new Array(size)
    .fill(1)
    .map((_, i) => i + skip + 1)
    .join();
  return fetchEpisodesRange(range);
};
...
export const fetchMoreCharacters = async ({ next }: {| +next: string |}): SearchResult => {
  try {
